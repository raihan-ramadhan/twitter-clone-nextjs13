...
export const LeftProfil = (): JSX.Element => {
  ...
  return (
    <>
      <div className="hidden xs:block absolute bottom-0 -inset-x-2 xs:w-20 sm:w-24 xl:w-[275px] z-10">
        <Menu className="flex justify-center" as="section">
          {({ open }): JSX.Element => (
            <>
              <div
                onClick={() => setMyOverlay(false)}
                className={cn(
                  "fixed z-20 inset-0 bg-transparent",
              ...
              <AnimatePresence>
                {open && (
                  <Menu.Items
                    className="py-3 z-30 absolute w-[275px] xl:w-[300px] left-0 xl:left-1/2 xl:-translate-x-1/2 -top-52 bg-white drop-shadow-[0_0_5px_rgba(0,0,0,0.15)] rounded-2xl"
                    as={motion.div}
                    {...variants}
                    static
                  >
                 ...
                 border-light-border px-4 py-3 dark:border-dark-border"
                    ...
                    <Menu.Item>
                      {({ active }): JSX.Element => (
                        <Button
                          className={cn(
                            "flex w-full gap-3 p-4 font-semibold",
                            active && "bg-black/5"
                          )}
                          onClick={() => {
                            console.log("TEST");
